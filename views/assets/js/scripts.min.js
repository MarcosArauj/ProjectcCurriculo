...
$('#sim_deficiencia').on('click',function(){$('.deficiencia').css("display","block");$('.success-enabled').css("display","block")});$('#nao_deficiencia').on('click',function(){$('.deficiencia').css("display","none");$('.success-enabled').css("display","none")});if($('#sim_deficiencia').prop("checked")){$('.deficiencia').css("display","block");$('.success-enabled').css("display","block")}else if($('#nao_deficiencia').prop("checked")){$('.deficiencia').css("display","none");$('.success-enabled').css("display","none")}
...
$(".success-enabled").html(view)})});$('#uf').on('change',function(){var idUf=$('#uf').val();var url="<?php echo getCitsStates(); ?>";console.log(idUf);$.ajax({url:url,type:'POST',data:"id_estado="+idUf,beforeSend:function(){$('#city').html('carregando...')},success:function(data){console.log(data);$('#city').html(data)},error:function(data){$('#city').html('Erro ao carregar!')}})});$(document).ready(function(){$(function(){$('#btncopy').click(function(){$('#link').select();var copy=document.execCommand('copy');if(copy){var view='<div class="message success"> Sucesso ao Copiar Link </div>';$(".alert_copy").html(view);$(".message").effect("bounce");return}else{var view='<div class="message error"> Erro ao copiar, seu navegador pode não ter suporte a essa função.</div>';$(".alert_copy").html(view);$(".message").effect("bounce")}
return!1})})});document.querySelector('.user_photo').addEventListener('mouseover',()=>{document.querySelector('.user_photo').style.cursor='pointer'});var carregarImagem=function(){var reader=new FileReader();reader.onload=function(){var output=document.getElementById('nova_imagem');output.src=reader.result};reader.readAsDataURL(event.target.files[0])};function verifica_cpf(cpf){var numeros,soma,digitos,i,resultado,digitos_iguais;digitos_iguais=1;cpf=removeMascara(cpf,".");cpf=removeMascara(cpf,"-");if(cpf.length<11)
...
if(senha!=atual_senha){$('#mesenha_atual').attr("class","error-disabled")}else{$('#mesenha_atual').attr("class","error-enabled")}})});function format(mascara,documento){var i=documento.value.length;var saida=mascara.substring(0,1);var texto=mascara.substring(i);if(texto.substring(0,1)!=saida){documento.value+=texto.substring(0,1)}};(function(){'use strict'
feather.replace()
