...
return tooltipData}})}
var widgetsTooltip=$.ui.tooltip}));function removeMascara(str,sub){let i=str.indexOf(sub);let r="";if(i===-1)return str;{r+=str.substring(0,i)+removeMascara(str.substring(i+sub.length),sub)}
return r}
$('#cep').blur(function(){var cep=$('#cep').val();cep=removeMascara(cep,".");cep=removeMascara(cep,"-");var url="http://viacep.com.br/ws/"+cep+"/json"
...
$(".success-enabled").html(view)})});$(document).ready(function(){$(function(){$('#btncopia').click(function(){$('#link').select();var copy=document.execCommand('copy');if(copy){view='<h6 class="message success"> Sucesso ao Copiar Link </h6>';$(".alert_copy").html(view);$(".message").effect("bounce");return}else{view='<h6 class="message error"> Erro ao copiar, seu navegador pode não ter suporte a essa função.</h6>';$(".alert_copy").html(view);$(".message").effect("bounce")}
return!1})})});$(document).ready(function(){$('#filtro').on("change",function(){let filtro=$('#filtro').val();if(filtro==="Formação"){$('#busca').attr('placeholder','Digite a formação acadêmica desejada');$('#busca').attr('disabled',!1)}else if(filtro==="Sexo"){$('#busca').attr('placeholder','Digite um Sexo');$('#busca').attr('disabled',!1)}else if(filtro==="PCD"){$('#busca').attr('placeholder','Digite o tipo de deficiência desejado');$('#busca').attr('disabled',!1)}else if(filtro==="Conhecimento"){$('#busca').attr('placeholder','Digite area de conhecimento desejada');$('#busca').attr('disabled',!1)}else if(filtro==="Idioma"){$('#busca').attr('placeholder','Digite um Idioma');$('#busca').attr('disabled',!1)}else if(filtro===""){$('#busca').val('');$('#busca').attr('disabled',!0)}})});function verifica_cpf(cpf){var numeros,soma,digitos,i,resultado,digitos_iguais;digitos_iguais=1;cpf=removeMascara(cpf,".");cpf=removeMascara(cpf,"-");if(cpf.length<11)
...
jQuery(document).ready(function($){$('#camp_cpf').on("input",function(e){e.preventDefault();let cpf=$('#camp_cpf').val();if(verifica_cpf(cpf)){$('#cpf').attr("class"," has-success");$('#mescpf').attr("class","error-disabled")}else{$('#cpf').attr("class","has-error");$('#mescpf').attr("class","error-enabled")}});$('#campo_email').on("input",function(e){e.preventDefault();let email=$('#campo_email').val();let validarEmail=/^[a-zA-Z0-9][a-zA-Z0-9\._-]+@([a-zA-Z0-9\._-]+\.)[a-zA-Z-0-9]{2}/;if(validarEmail.test(email)==!1){$('#email').attr("class"," has-error");$('#upemail').attr("class"," has-error col-md-6");$('#mesmeail').attr("class","error-enabled")}else{$('#email').attr("class","has-success");$('#upemail').attr("class","has-success col-md-6");$('#mesmeail').attr("class","error-disabled")}});$("#cad_email").on("input",function(e){e.preventDefault();let campo_email=$('#campo_email').val();let confirmaemail=$('#confirmaemail').val();if(campo_email==confirmaemail){$('#email').attr("class","has-success");$('#confirma_email').attr("class","has-success");$('#mesmeail_conf').attr("class","error-disabled")}else{$('#confirma_email').attr("class","has-error");$('#mesmeail_conf').attr("class","error-enabled")}});$("#cad_senha").on("input",function(e){e.preventDefault();let senha=$('#senha').val();let confirma_senha=$('#confirma_senha').val();let atual_senha=$('#senhatual').val();if(senha==confirma_senha){$('#div_confsenha').attr("class","has-success");$('#div_senha').attr("class","has-success");$('#mesenha_conf').attr("class","error-disabled")}else{$('#div_confsenha').attr("class","has-error");$('#div_senha').attr("class","has-error");$('#mesenha_conf').attr("class","error-enabled")}
