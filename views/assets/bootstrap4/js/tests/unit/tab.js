$(function () {
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    ...
    var tabsHTML = '<ul class="nav">' +
        '<li><a href="#web">WebController</a></li>' +
        '<li><a href="#profile">Profile</a></li>' +
        '</ul>'

    $('<ul><li id="web"/><li id="profile"/></ul>').appendTo('#qunit-fixture')

    $(tabsHTML).find('li:last-child a').bootstrapTab('show')
    assert.strictEqual($('#qunit-fixture').find('.active').attr('id'), 'profile')

    $(tabsHTML).find('li:first-child a').bootstrapTab('show')
    assert.strictEqual($('#qunit-fixture').find('.active').attr('id'), 'web')
  })
  ...
  QUnit.test('should activate element by tab id in ordered list', function (assert) {
    ...
    $('<ol><li id="web"/><li id="profile"/></ol>').appendTo('#qunit-fixture')
  ...
  QUnit.test('should activate element by tab id in nav list', function (assert) {
    ...
    var tabsHTML =  '<nav class="nav">' +
                      '<a href="#web">WebController</a>' +
                      '<a href="#profile">Profile</a>' +
                    '</nav>'

    $('<nav><div id="web"></div><div id="profile"></div></nav>').appendTo('#qunit-fixture')
  ...
  QUnit.test('should not fire shown when tab is already active', function (assert) {
    ...
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">' +
      '<li class="nav-item" role="presentation"><a href="#web" class="nav-link active" role="tab">WebController</a></li>' +
      '<li class="nav-item" role="presentation"><a href="#profile" class="nav-link" role="tab">Profile</a></li>' +
      '</ul>' +
      '<div class="tab-content">' +
      '<div class="tab-pane active" id="web" role="tabpanel"></div>' +
  ...
  QUnit.test('should not fire shown when tab is disabled', function (assert) {
    assert.expect(0)
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">' +
      '<li class="nav-item"><a href="#web" class="nav-link active" role="tab">WebController</a></li>' +
  ...
  QUnit.test('selected tab should have aria-selected', function (assert) {
    ...
    var tabsHTML = '<ul class="nav nav-tabs">' +
        '<li><a class="nav-item active" href="#web" toggle="tab" aria-selected="true">WebController</a></li>' +
  ...
  QUnit.test('selected tab should deactivate previous selected tab', function (assert) {
    assert.expect(2)
    var tabsHTML = '<ul class="nav nav-tabs">' +
        '<li class="nav-item"><a class="nav-link active" href="#web" data-toggle="tab">WebController</a></li>' +
  ...
  QUnit.test('selected tab should deactivate previous selected link in dropdown', function (assert) {
    assert.expect(3)
    var tabsHTML = '<ul class="nav nav-tabs">' +
        '<li class="nav-item"><a class="nav-link" href="#web" data-toggle="tab">WebController</a></li>' +
        '<li class="nav-item"><a class="nav-link" href="#profile" data-toggle="tab">Profile</a></li>' +
  ...
  QUnit.test('should not remove fade class if no active pane is present', function (assert) {
    ...
    var done = assert.async()
    var tabsHTML = '<ul class="nav nav-tabs" role="tablist">' +
      '<li class="nav-item" role="presentation"><a id="tab-web" href="#web" class="nav-link" data-toggle="tab" role="tab">WebController</a></li>' +
      '<li class="nav-item" role="presentation"><a id="tab-profile" href="#profile" class="nav-link" data-toggle="tab" role="tab">Profile</a></li>' +
      '</ul>' +
      '<div class="tab-content">' +
      '<div class="tab-pane fade" id="web" role="tabpanel"></div>' +
      ...
      .on('shown.bs.tab', function () {
        ...
        $('#tab-web')
          .on('shown.bs.tab', function () {
            ...
            assert.ok($('#web').hasClass('fade'))
            assert.ok($('#web').hasClass('show'))
  ...
  QUnit.test('should not add show class to tab panes if there is no `.fade` class', function (assert) {
    ...
    var html = [
      '<ul class="nav nav-tabs" role="tablist">',
      '  <li class="nav-item" role="presentation">',
      '    <a class="nav-link nav-tab" href="#web" role="tab" data-toggle="tab">WebController</a>',
      '  </li>',
      '  <li class="nav-item" role="presentation">',
      ...
      '  <div role="tabpanel" class="tab-pane" id="web">test 1</div>',
  ...
  QUnit.test('should add show class to tab panes if there is a `.fade` class', function (assert) {
    ...
    var html = [
      ...
      '</ul>',
      '<div class="tab-content">',
      '  <div role="tabpanel" class="tab-pane fade" id="web">test 1</div>',
